...
"""
from fastapi import FastAPI, HTTPException, Depends, Body, Request, Security, Header
...
from llamarag.core.chunking import Chunker, ParagraphChunker, SentenceChunker, FixedSizeChunker
...
CORS_ORIGINS = os.environ.get("LLAMARAG_CORS_ORIGINS", "").split(",") if os.environ.get("LLAMARAG_CORS_ORIGINS") else []
...
async def chunk_text(request: ChunkRequest, api_key: str = Depends(verify_api_key)):
    ...
    if request.max_chunk_size is None:
        ...
        if chunker_type == "paragraph":
            ...
            chunker = FixedSizeChunker(chunk_size=request.max_chunk_size, chunk_overlap=overlap)
...
async def add_documents(documents_list: List[DocumentModel], api_key: str = Depends(verify_api_key)):
