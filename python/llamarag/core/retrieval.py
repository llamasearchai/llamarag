...
class HybridRetriever(Retriever):
    ...
    def __init__(self, retrievers: List[Tuple[Retriever, float]]):
        """
            ...
            for result in results:
                ...
                if doc_id in all_results:
                    ...
                    weighted_result = SearchResult(
                        result.document, result.score * weight, result.rank
                    )
...
class RerankingRetriever(Retriever):
    ...
    def __init__(
        self, base_retriever: Retriever, reranker, candidate_multiplier: int = 3
    ):
        """
    ...
    def retrieve(self, query: str, k: int = 3) -> List[SearchResult]:
        ...
        candidates = self.base_retriever.retrieve(
            query, k=k * self.candidate_multiplier
        )
